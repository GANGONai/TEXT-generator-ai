...
def build_models_tab(models: ModelManager) -> None:
    ...
    with gr.Row():
        ...
        with gr.Column():
            ...
            del_dd = gr.Dropdown(label="Модель", choices=_names(), interactive=True)
    ...
    with gr.Row():
        ...
        with gr.Column():
            ...
            exp_dd = gr.Dropdown(label="Модель", choices=_names(), interactive=True)
    ...
    # ---- callbacks ----
    def refresh_all() -> Tuple[Any, Any, Any]:
        return _table_data(), gr.update(choices=_names()), gr.update(choices=_names())

    refresh_btn.click(fn=refresh_all, outputs=[model_table, del_dd, exp_dd])

    def create_model(name: str, base: str) -> Tuple[Any, str, Any, Any]:
        ...
        return (
            ...
            gr.update(choices=_names()),
            gr.update(choices=_names()),
        )

    create_btn.click(fn=create_model, inputs=[new_name, base_dd], outputs=[model_table, create_status, del_dd, exp_dd])

    def delete_model(name: str) -> Tuple[Any, str, Any, Any]:
        if not name:
    ...
    del_btn.click(fn=delete_model, inputs=del_dd, outputs=[model_table, del_status, del_dd, exp_dd])
    ...
    def do_import(file_obj: Any, rename: str, overwrite: bool) -> Tuple[Any, str, Any, Any]:
        if file_obj is None:
    ...
    imp_btn.click(
        fn=do_import,
        inputs=[imp_file, imp_rename, imp_overwrite],
