...
def build_upload_tab(models: ModelManager, trainer: LyricsTrainer) -> None:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=2):
            ...
            with gr.Accordion("Как должен выглядеть TXT / CSV", open=False):
                gr.Markdown(
                    "**TXT — несколько песен с названиями и жанрами.** "
                    "Каждая песня — блок, между блоками **две пустые строки**. "
                    "В начале блока — необязательные строки `Название:`, "
                    "`Жанр:`, `Текст:` (можно по-английски: `Title:`, "
                    "`Genre:`, `Lyrics:`). Всё после `Текст:` (или сразу "
                    "после последнего заголовка) считается текстом песни.\n\n"
                    "```\n"
                    ...
                    "```\n\n"
                    "Без заголовков тоже работает: блоки, разделённые двумя "
                    "пустыми строками, импортируются как «Песня #1», «#2»… "
                    "Без разделителей весь файл — одна песня.\n\n"
                    "**CSV** — UTF-8 с заголовком первой строкой. Обязательная "
                    "колонка одна — `text`. Опционально `title` и `genre`. "
                    "Многострочный текст оборачивайте в двойные кавычки `\"…\"`."
                )
            with gr.Row():
